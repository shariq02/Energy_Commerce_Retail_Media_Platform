...
# ///
# MAGIC %md
# MAGIC # BRONZE DATA LOADING -- POWER PLANT LIST (ENERGY)
# MAGIC
...
# MAGIC
# MAGIC **Purpose:** Load uploaded Unity Catalog Volume data for the BNetzA
# MAGIC power plant list (energy domain) into the frozen Bronze table structure.
...
SOURCE_VOLUME = "power_plant_list_analytical"
...
DATASETS: list[tuple[str, str, str]] = [
    (
        "kraftwerksliste",
        SOURCE_VOLUME,
        f"{CATALOG}.{BRONZE_SCHEMA}.power_plant_list",
    ),
    (
        "zu_und_rueckbau",
        SOURCE_VOLUME,
        f"{CATALOG}.{BRONZE_SCHEMA}.power_plant_capacity_additions",
    ),
]
...
def dataset_file_pattern(dataset: str) -> re.Pattern:
    # Both datasets are staged as a single flat file, no chunking.
    return re.compile(rf"^{re.escape(dataset)}\.csv$")
...
# DBTITLE 1,Power plant list Bronze load summary
print("=" * 70)
print("POWER PLANT LIST BRONZE LOAD SUMMARY")
print("=" * 70)
for result in load_results:
    print(
        f"{result['dataset']:<20} files={result['files']:<3} "
...
if not overall_success:
    raise RuntimeError(
        "FAIL  Power plant list Bronze load did not fully pass -- see summary above. "
