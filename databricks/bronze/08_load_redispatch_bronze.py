...
# ///
# MAGIC %md
# MAGIC # BRONZE DATA LOADING -- REDISPATCH (ENERGY)
# MAGIC
...
# MAGIC
# MAGIC **Purpose:** Load uploaded Unity Catalog Volume data for Redispatch
# MAGIC (energy domain) into the frozen Bronze table structure.
...
DATASETS: list[tuple[str, str, str]] = [
    (
        ...
        f"{CATALOG}.{BRONZE_SCHEMA}.redispatch_measures",
    ),
]
...
def dataset_file_pattern(dataset: str) -> re.Pattern:
    # Staged as a single flat file, no chunking.
...
# DBTITLE 1,Redispatch Bronze load summary
print("=" * 70)
print("REDISPATCH BRONZE LOAD SUMMARY")
print("=" * 70)
for result in load_results:
    print(
...
if not overall_success:
    raise RuntimeError(
        "FAIL  Redispatch Bronze load did not fully pass -- see summary above. "
