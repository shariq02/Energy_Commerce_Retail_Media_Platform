...
# ///
# MAGIC %md
# MAGIC # BRONZE DATA LOADING -- GA4 (COMMERCE / DIGITAL BEHAVIOUR)
# MAGIC
...
# MAGIC
# MAGIC **Purpose:** Load the uploaded Unity Catalog Volume data (the scoped,
# MAGIC filtered/projected staging output, not the raw archive) into the
# MAGIC Bronze table. Nested fields (`event_params`, `ecommerce`, `items`) are
# MAGIC kept as Delta STRUCT/ARRAY, inferred from the JSON -- not flattened to
# MAGIC STRING, since they're structured data, not a flat source.
...
DATASETS: list[tuple[str, str, str]] = [
    ("events", "ga4_events", f"{CATALOG}.{BRONZE_SCHEMA}.ga4_events"),
]

VOLUMES = sorted({volume for _, volume, _ in DATASETS})

FILE_EXT_PATTERN = r"jsonl"
...
def dataset_file_pattern(dataset: str) -> re.Pattern:
    # Matches "<dataset>.jsonl", an optionally "<source>_"-prefixed variant,
    # or a physically chunked upload "<dataset>_chunk_00001.jsonl". Chunk
...
def read_dataset(files: list[str]) -> DataFrame:
    # One compact JSON object per line (our own staging writer) -- multiLine
    # off keeps files splittable and parses correctly.
    return spark.read.option("multiLine", "false").json(files)
...
# DBTITLE 1,GA4 Bronze load summary
print("=" * 70)
print("GA4 BRONZE LOAD SUMMARY")
print("=" * 70)
for result in load_results:
    print(
...
if not overall_success:
    raise RuntimeError(
        "FAIL  GA4 Bronze load did not fully pass -- see summary above. "
