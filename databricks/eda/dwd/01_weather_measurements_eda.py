...
PLAUSIBLE = {
    "TT_TU": (-40.0, 45.0), "RF_TU": (0.0, 100.0), "TF_STD": (-40.0, 45.0),
    "P": (900.0, 1080.0), "P0": (900.0, 1080.0), "N": (0.0, 8.0), "V_N": (0.0, 8.0),
    "R1": (0.0, 200.0), "RS_IND": (0.0, 1.0), "SD_SO": (0.0, 60.0), "F": (0.0, 60.0), "D": (0.0, 360.0),
}
...
for m in MEASUREMENTS:
    ...
    exprs = [F.count(F.lit(1)).alias("__rows"),
             F.min(find_col(df, "MESS_DATUM")).alias("__min_ts"), F.max(find_col(df, "MESS_DATUM")).alias("__max_ts")]
    for c in cols:
    ...
    prof[m] = {"cols": cols, "total": r["__rows"], "min_ts": r["__min_ts"], "max_ts": r["__max_ts"],
    ...
    for c in cols:
        print(f"  {c:<26} missing={r[c + '__m']:>12} rate={r[c + '__m'] / r['__rows']:.4f} approx_distinct={r[c + '__d']}")
    print("constant columns:", [c for c in cols if r[c + "__d"] <= 1])
totals = {m: prof[m]["total"] for m in MEASUREMENTS}
coverage = {m: {"stations": prof[m]["acd"][find_col(frames[m], "STATIONS_ID")],
                "cities": prof[m]["acd"][find_col(frames[m], "city")],
                "min_ts": prof[m]["min_ts"], "max_ts": prof[m]["max_ts"]} for m in MEASUREMENTS}
constant_cols = {m: [c for c in prof[m]["cols"] if prof[m]["acd"][c] <= 1] for m in MEASUREMENTS}
...
for m in MEASUREMENTS:
    ...
    for c in value_columns(df):
        ...
        b = PLAUSIBLE.get(c.upper())
        exprs += [
            F.min(v).alias(c + "_min"), F.max(v).alias(c + "_max"),
            F.min(F.when(v != -999, v)).alias(c + "_min_ns"), F.max(F.when(v != -999, v)).alias(c + "_max_ns"),
            F.sum((v == -999).cast("long")).alias(c + "_sentinel"),
            F.avg(F.when(v != -999, v)).alias(c + "_mean"), F.stddev(F.when(v != -999, v)).alias(c + "_sd"),
            F.expr(f"percentile_approx(case when cast(`{c}` as double) != -999 then cast(`{c}` as double) end, array(0.01,0.5,0.99))").alias(c + "_p"),
            F.sum((v == 0).cast("long")).alias(c + "_zero"),
            *([F.sum(((v != -999) & ((v < b[0]) | (v > b[1]))).cast("long")).alias(c + "_oor")] if b else []),
        ]
    value_stats[m] = df.agg(*exprs).first().asDict()
    for c in value_columns(df):
        print(f"{m}.{c:<12}", {k[len(c) + 1:]: value_stats[m][k] for k in value_stats[m] if k.startswith(c + "_")})
...
for m in MEASUREMENTS:
    ...
    per_station = (
        ...
        .where(F.col("ts").isNotNull()).distinct()
        .withColumn("gap_h", (F.col("ts").cast("long") - F.lag("ts").over(w).cast("long")) / 3600 - 1)
        .groupBy("station").agg(
            F.min("ts").alias("min_ts"), F.max("ts").alias("max_ts"),
            F.count(F.lit(1)).alias("observed_hours"),
            F.max(F.when(F.col("gap_h") > 0, F.col("gap_h"))).alias("longest_gap_hours"),
            F.sum(F.when(F.col("gap_h") > 0, F.col("gap_h")).otherwise(0)).alias("total_missing_hours"),
        )
        .withColumn("expected_hours",
                    ((F.col("max_ts").cast("long") - F.col("min_ts").cast("long")) / 3600 + 1).cast("long"))
        .withColumn("coverage_pct", F.round(F.col("observed_hours") / F.col("expected_hours") * 100, 2))
        .orderBy("station")
    )
...
coverage_matrix = [{"station": s, **{m: station_counts[m].get(s, 0) for m in MEASUREMENTS}} for s in all_stations]
...
for m in MEASUREMENTS:
    ...
    vcols = value_columns(df)
    value_pdf[m] = (
        df.select(*[F.when(F.col(c).cast("double") != -999, F.col(c).cast("double")).alias(c) for c in vcols])
        .sample(0.05, seed=42).limit(150_000).toPandas()
    )
...
barplot([(m, totals[m]) for m in MEASUREMENTS], "DWD -- rows per measurement", "measurement", rot=30)
barplot([(m, coverage[m]["stations"]) for m in MEASUREMENTS], "DWD -- distinct stations per measurement", "measurement", "stations", rot=30)
barplot([(m, coverage[m]["cities"]) for m in MEASUREMENTS], "DWD -- distinct cities per measurement", "measurement", "cities", rot=30)
plt.figure(figsize=(10, 4))
for i, m in enumerate(MEASUREMENTS):
    plt.plot([int(str(coverage[m]["min_ts"])[:4]), int(str(coverage[m]["max_ts"])[:4])], [i, i], marker="|", markersize=12)
plt.yticks(range(len(MEASUREMENTS)), MEASUREMENTS)
plt.title("DWD -- observation year span per measurement"); plt.xlabel("year")
plt.tight_layout(); plt.show()
...
plt.bar(x - 0.2, [dup_breakdown[m]["identical"] for m in MEASUREMENTS], width=0.4, label="fully-identical repeat groups")
plt.bar(x + 0.2, [dup_breakdown[m]["conflicting"] for m in MEASUREMENTS], width=0.4, label="keys with conflicting rows")
plt.xticks(x, MEASUREMENTS, rotation=30, ha="right"); plt.legend()
plt.title("DWD -- duplicate key composition"); plt.ylabel("key groups"); plt.tight_layout(); plt.show()
for m in MEASUREMENTS:
    barplot([(r["station"], r["coverage_pct"]) for r in freq_cov[m]],
            f"DWD {m} -- hourly coverage % per station", "station id", "coverage %", rot=45)
    barplot([(r["station"], r["longest_gap_hours"] or 0) for r in freq_cov[m]],
            f"DWD {m} -- longest missing-hours gap per station", "station id", "hours", rot=45)

# COMMAND ----------

# DBTITLE 1,Figure -- station x measurement coverage heatmap
grid = np.array([[row[m] for m in MEASUREMENTS] for row in coverage_matrix], dtype=float)
plt.figure(figsize=(9, max(3, 0.4 * len(all_stations))))
plt.imshow(np.where(grid > 0, np.log10(grid + 1), np.nan), aspect="auto", cmap="viridis")
...
for m in MEASUREMENTS:
    ...
    for c in cols:
        histplot(pdf[c].dropna().tolist(), f"DWD {m}.{c} -- value distribution (sampled)", c)
    if cols:
        plt.figure(figsize=(max(6, 1.6 * len(cols)), 4))
        plt.boxplot([pdf[c].dropna().tolist() for c in cols], labels=cols, showfliers=True)
        plt.title(f"DWD {m} -- value column spread (sampled)"); plt.ylabel("value")
        plt.tight_layout(); plt.show()
...
for m in MEASUREMENTS:
    ...
    b = dup_breakdown[m]
    print(f"{m}: rows={totals[m]}, stations={coverage[m]['stations']}, "
