...
for c in VCOLS:
    ...
    exprs += [
        ...
        F.expr(f"percentile_approx(cast(`{c}` as double), array(0.01,0.5,0.99))").alias(c + "_p"),
        F.sum((v == 0).cast("long")).alias(c + "_zero"),
        F.sum((F.col(c).isNotNull() & (F.trim(F.col(c)) != "") & v.isNull()).cast("long")).alias(c + "_non_numeric"),
        *([F.sum(((v < b[0]) | (v > b[1])).cast("long")).alias(c + "_out_of_range")] if b else []),
    ]
...
for c in COLS:
    print(f"  {c:<32} missing={S[c + '__m']:>10} rate={S[c + '__m'] / total:.4f} approx_distinct={S[c + '__d']}")
...
for freq, step in FREQ_SECONDS.items():
    ...
    missing = sum((x["delta_s"] / step - 1) * x["count"] for x in fg if x["delta_s"] and x["delta_s"] > step)
    expected = observed + missing
    continuity.append((freq, observed, round(observed / expected * 100, 2) if expected else None,
                       round(on_step / intervals * 100, 2) if intervals else None, round(longest_gap, 1)))
    print(continuity[-1])
delta_dist = sorted(((x["frequency"], x["delta_s"], x["count"]) for x in g), key=lambda t: -t[2])[:20]
...
for c in VCOLS:
    ...
    stuck_exprs.append(F.sum(
        (v.isNotNull() & (v == F.lag(v, 1).over(w2)) & (v == F.lag(v, 11).over(w2))).cast("long")
    ).alias(c))
...
ts_pdf = (
    ...
    .orderBy("datetime_utc").limit(3000).toPandas()
)
hourly = (
    df.where(F.col("frequency") == "1h")
    .groupBy(F.hour(F.to_timestamp("datetime_utc")).alias("hod"))
    .agg(*[F.avg(F.col(c).cast("double")).alias(c) for c in VCOLS]).orderBy("hod").collect()
)
...
if continuity:
    barplot([(r[0], r[2]) for r in continuity], "Honda weather -- coverage % by frequency", "frequency", "%")
    barplot([(r[0], r[4]) for r in continuity], "Honda weather -- longest gap (steps) by frequency", "frequency", "steps")
...
for c in VCOLS:
    ...
    if len(s):
        histplot(s.tolist(), f"Honda weather.{c} -- distribution (n={len(s)} sample)", c)
if not ts_pdf.empty:
...
for c in VCOLS:
    ...
    plt.title(f"Honda weather -- mean {c} by hour of day"); plt.xlabel("hour"); plt.ylabel(c)
    plt.tight_layout(); plt.show()
...
print("value plausibility (range / out_of_range / zero):",
      {c: {k[len(c) + 1:]: S[k] for k in S if k.startswith(c + "_") and ("out_of_range" in k or "zero" in k or "min" in k or "max" in k)} for c in VCOLS})
