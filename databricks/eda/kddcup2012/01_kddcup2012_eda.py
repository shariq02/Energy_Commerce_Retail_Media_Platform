...
ID_COLS = ["url_hash", "ad_id", "advertiser_id", "query_id", "keyword_id",
...
for c in COLS:
    print(f"  {c:<16} missing={r[c + '__m']:>10} rate={r[c + '__m'] / total:.4f} approx_distinct={r[c + '__d']}")
...
agg = [
    F.sum(clk).alias("clicks"), F.sum(imp).alias("impressions"),
    F.sum((F.col("click").cast("int") > 0).cast("long")).alias("positive_rows"),
    F.sum((clk > imp).cast("long")).alias("click_gt_impr"),
    F.sum((F.col("position").cast("int") > F.col("depth").cast("int")).cast("long")).alias("pos_gt_depth"),
]
for c in ZERO_SENTINEL:
    agg.append(F.sum(((F.col(c) == "0") | (F.col(c) == 0)).cast("long")).alias(c + "__zero"))
...
print(f"class: positive={pos} ({pos / total:.4%})  negative={neg} ({neg / total:.4%})  "
      f"imbalance neg:pos = {neg / max(pos, 1):.1f}:1")
print("invalid click>impression:", S["click_gt_impr"], " position>depth:", S["pos_gt_depth"])
print("id-0 (unknown) rows:", {c: (n, round(n / total, 4)) for c, n in sentinel_zero.items()})

# COMMAND ----------

# DBTITLE 1,Small-domain distributions -- click / impression / depth / position + CTR (one groupBy)
g = df.groupBy("click", "impression", "depth", "position").agg(
...
kg = df.groupBy(*CONTEXT).agg(
    F.count(F.lit(1)).alias("n"), F.countDistinct("click").alias("distinct_click"),
)
kb = kg.agg(
    F.sum((F.col("n") > 1).cast("long")).alias("dup_keys"),
    F.sum(((F.col("n") > 1) & (F.col("distinct_click") > 1)).cast("long")).alias("conflicting"),
).first().asDict()
print(f"context keys with >1 row: {kb['dup_keys']}  of which conflicting click labels: {kb['conflicting']}")
kg.where((F.col("n") > 1) & (F.col("distinct_click") > 1)).orderBy(F.desc("n")).limit(20).show(truncate=False)
...
def card(parent, *children):
    ...
    for c in children:
        s = d.agg(F.min(c).alias("min"), F.max(c).alias("max"), F.avg(c).alias("avg")).first().asDict()
...
for c in ("ad_id", "advertiser_id", "query_id", "user_id"):
    top = df.groupBy(c).agg(F.count(F.lit(1)).alias("rows"),
                            F.avg(F.col("click").cast("double")).alias("ctr")) \
            .orderBy(F.desc("rows")).limit(5000).collect()
...
# DBTITLE 1,Figures
barplot([("click=0", neg), ("click>0", pos)], "KDD -- click class balance", "class", "rows")
barplot([(c, n / total) for c, n in sentinel_zero.items()],
        "KDD -- share of rows with id value 0 (unknown)", "column", "rate", rot=30)
...
plt.title("KDD -- distinct value count per id column"); plt.ylabel("distinct (log)")
plt.xticks(rotation=30, ha="right"); plt.tight_layout(); plt.show()
for c in ("ad_id", "query_id", "user_id"):
    counts = top_counts[c]
    plt.figure(figsize=(9, 4))
    plt.loglog(range(1, len(counts) + 1), counts, marker=".", linestyle="none")
    plt.title(f"KDD -- rows per {c} (top 5000, log-log)"); plt.xlabel("rank"); plt.ylabel("rows")
    plt.tight_layout(); plt.show()
...
# DBTITLE 1,Findings
print("constant columns:", constant_cols)
print("id-0 (unknown) rates:", {c: round(n / total, 4) for c, n in sentinel_zero.items()})
