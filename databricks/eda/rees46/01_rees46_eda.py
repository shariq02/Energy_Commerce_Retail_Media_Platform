...
for c in COLS:
    ...
    prof_exprs += [F.sum(miss.cast("long")).alias(c + "__m"), F.approx_count_distinct(c).alias(c + "__d")]
...
for c in COLS:
    print(f"  {c:<16} missing={r[c + '__m']:>14} rate={missrates[c]:.4f} approx_distinct={r[c + '__d']}")
print("constant columns:", constant_cols)
print("cardinality is approximate (approx_count_distinct); exact where a check needs it below")
...
day_type = (
    ...
    .count().orderBy("day", "event_type")
).collect()
days = sorted({x["day"] for x in day_type})
event_types = sorted({x["event_type"] for x in day_type})
funnel = [(et, sum(x["count"] for x in day_type if x["event_type"] == et)) for et in event_types]
...
by_type = df.groupBy("event_type").agg(
    F.min(price).alias("price_min"), F.max(price).alias("price_max"), F.avg(price).alias("price_avg"),
    F.expr("percentile_approx(cast(price as double), array(0.5, 0.95, 0.99))").alias("price_p50_95_99"),
    F.sum(F.when(F.col("price").isNull() | (F.trim(F.col("price")) == ""), 1).otherwise(0)).alias("price_missing"),
    F.sum(F.when(F.col("price").isNotNull() & (F.trim(F.col("price")) != "") & price.isNull(), 1).otherwise(0)).alias("price_non_numeric"),
    F.sum(F.when(price <= 0, 1).otherwise(0)).alias("price_le_zero"),
    F.sum(F.when(price < 0, 1).otherwise(0)).alias("price_negative"),
    F.sum(F.when(F.col("category_code").isNull() | (F.trim(F.col("category_code")) == ""), 1).otherwise(0)).alias("category_code_missing"),
    F.sum(F.when(F.col("brand").isNull() | (F.trim(F.col("brand")) == ""), 1).otherwise(0)).alias("brand_missing"),
...
print(f"overall p99 price={price_p99}  zero-price purchases={zero_purch}  negative price rows={neg_price}")

# COMMAND ----------

# DBTITLE 1,Top brands / categories by event volume
top_brands = [(x["brand"], x["count"]) for x in
              df.groupBy("brand").count().orderBy(F.desc("count")).limit(20).collect()]
top_cats = [(x["category_code"], x["count"]) for x in
            df.groupBy("category_code").count().orderBy(F.desc("count")).limit(20).collect()]
...
print(f"products with >1 category_id: {n_multi_cat}   products with >1 brand: {n_multi_brand}")

cc = df.groupBy("category_id").agg(F.approx_count_distinct("category_code").alias("codes")) \
       .agg(F.sum((F.col("codes") > 1).cast("long"))).first()[0]
cc2 = df.where(F.trim(F.coalesce(F.col("category_code"), F.lit(""))) != "") \
        .groupBy("category_code").agg(F.approx_count_distinct("category_id").alias("ids")) \
        .agg(F.sum((F.col("ids") > 1).cast("long"))).first()[0]
print(f"category_ids mapping to >1 category_code: {cc}   category_codes mapping to >1 category_id: {cc2}")
...
sc = session_roll.agg(
    ...
    F.expr("approx_percentile(events, array(0.5, 0.9, 0.99))").alias("events_p50_90_99"),
    ...
    F.sum((F.col("has_view") & F.col("has_cart")).cast("int")).alias("view_and_cart"),
    F.sum((F.col("has_cart") & F.col("has_purchase")).cast("int")).alias("cart_and_purchase"),
    F.sum((F.col("has_view") & F.col("has_cart") & F.col("has_purchase")).cast("int")).alias("full_path"),
).first().asDict()
print(sc)
if sc["with_view"]:
    print(f"view->cart session rate     = {sc['view_and_cart'] / sc['with_view']:.4f}")
if sc["with_cart"]:
    print(f"cart->purchase session rate = {sc['cart_and_purchase'] / sc['with_cart']:.4f}")
session_funnel = [("sessions", sc["sessions"]), ("with view", sc["with_view"]),
                  ("with cart", sc["with_cart"]), ("with purchase", sc["with_purchase"])]
session_events_sample = [x["events"] for x in
                         session_roll.select("events").sample(0.02, seed=42).limit(200_000).collect()]
...
ur = user_roll.agg(
    ...
    F.expr("approx_percentile(purchases, array(0.5, 0.9, 0.99))").alias("purchases_p50_90_99"),
...
rebought = (
    df.where(F.col("event_type") == "purchase").groupBy("user_id", "product_id").count()
...
seq = df.select("event_type", ts.alias("ts")).withColumn(
    "prior_types", F.collect_set("event_type").over(w.rowsBetween(Window.unboundedPreceding, -1)),
)
sqv = seq.agg(
    F.sum(((F.col("event_type") == "purchase") & ~F.array_contains(F.col("prior_types"), "cart")).cast("long")).alias("purchase_no_cart"),
    F.sum(((F.col("event_type") == "cart") & ~F.array_contains(F.col("prior_types"), "view")).cast("long")).alias("cart_no_view"),
).first()
purch_no_cart, cart_no_view = sqv["purchase_no_cart"], sqv["cart_no_view"]
print(f"purchases with NO prior cart in the session : {purch_no_cart} of {funnel_map.get('purchase', 0)}")
print(f"carts with NO prior view in the session     : {cart_no_view} of {funnel_map.get('cart', 0)}")

# COMMAND ----------

# DBTITLE 1,Anomaly -- same-timestamp bursts within a session
burst = (
    df.groupBy("user_session", "event_time").count().where(F.col("count") > 20)
    .select("user_session").distinct().count()
)
...
hod_dow = df.groupBy(F.hour(ts).alias("hod"), F.date_format(ts, "E").alias("dow")).count().collect()
hod = {}
dow = {}
for x in hod_dow:
...
print(f"duplicate {key} groups={db['dup_key_groups']}  identical={dup_identical}  conflicting={dup_conflicting}")
...
price_pdf = (
    ...
    .sample(0.02, seed=42).limit(250_000).toPandas()
)
...
barplot(session_funnel, "REES46 -- sessions reaching each funnel stage", "stage", "sessions")
...
barplot(list(missrates.items()), "REES46 -- missing rate per column", "column", "rate", rot=30)
...
plt.title("REES46 -- events per day"); plt.xlabel("day"); plt.ylabel("events")
plt.xticks(rotation=90); plt.tight_layout(); plt.show()

plt.figure(figsize=(13, 4))
for et in event_types:
    series = {x["day"]: x["count"] for x in day_type if x["event_type"] == et}
    plt.plot(days, [series.get(d, 0) for d in days], marker=".", label=et, linewidth=0.9)
plt.legend(); plt.title("REES46 -- events per day by type"); plt.xticks(rotation=90)
plt.tight_layout(); plt.show()
...
if len(price_pdf):
    histplot(price_pdf["price"].tolist(), f"REES46 price -- distribution (sampled, <=p99={price_p99})", "price")
    for et in event_types:
...
barplot(top_brands, "REES46 -- top 20 brands by event volume", "brand", "events", rot=90)
barplot(top_cats, "REES46 -- top 20 category_code by event volume", "category_code", "events", rot=90)
if session_events_sample:
    histplot(session_events_sample, "REES46 -- events per session (sampled)", "events in session", bins=60, log=True)
barplot([("multi-category products", n_multi_cat), ("multi-brand products", n_multi_brand)],
        "REES46 -- products with unstable category / brand", "", "products")
...
print("session funnel view/cart/purchase:", {k: sc[k] for k in ("with_view", "with_cart", "with_purchase", "view_and_cart", "cart_and_purchase", "full_path")})
print("repeat buyers:", ur["repeat_buyers"], "of", ur["buyers"], " (user,product) rebought:", rebought)
print("multi-session users:", ur["multi_session_users"], " multi-user sessions:", sc["multi_user_sessions"])
print("concentration:", concentration)
print("in-session sequence violations: purchase-no-cart =", purch_no_cart, " cart-no-view =", cart_no_view)
print("anomalies: >1000-event sessions =", sc["big_sessions"], " burst sessions =", burst,
      " zero-price purchases =", zero_purch, " negative price =", neg_price)
print("category stability: products multi-cat =", n_multi_cat, " multi-brand =", n_multi_brand,
      " cat_id->code conflicts =", cc, " code->cat_id conflicts =", cc2)
print("dup key: groups =", db["dup_key_groups"], " identical =", dup_identical, " conflicting =", dup_conflicting)
