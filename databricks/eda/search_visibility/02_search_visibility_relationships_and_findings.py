...
print(f"repository rows={repo_total}  columns={repo.columns}  key={repo_key}  key_unique={repo_key_unique}")
...
evr = events.groupBy(F.col("repository_id").cast("string").alias("repository_id")).agg(
    F.count(F.lit(1)).alias("events"),
    F.countDistinct("date").alias("distinct_dates"),
    F.min("date").alias("first_date"), F.max("date").alias("last_date"),
).orderBy(F.desc("events")).collect()
...
S = events.agg(
    ...
    F.sum(F.col("repository_id").cast("string").isin(repo_id_list).cast("long")).alias("matched_rows"),
).first().asDict()
ev_total = S["ev_total"]
...
print(f"events={ev_total}  distinct repository_id~={S['events_distinct_repo']}  matched rows={matched}  "
      f"unmatched rows={ev_total - matched}")
print(f"orphan event repository_ids (not in repository table)={orphan_events}  e.g. {sorted(event_repo_ids - repo_ids)[:20]}")
print(f"unused repository ids (never in events)={unused_repo}  e.g. {sorted(repo_ids - event_repo_ids)[:20]}")
print(f"events <-> repository cardinality: {kind}")
print(f"overall distinct dates={overall_dates}  repositories NOT present in every date={partial} of {len(event_repo_ids)}")

# COMMAND ----------

# DBTITLE 1,Figure -- coverage, integrity, events per repository
barplot([(x["repository_id"], x["distinct_dates"]) for x in evr],
        "Search Visibility -- distinct dates per repository_id", "repository_id", "dates", rot=90)
barplot([("repository ids", len(repo_ids)), ("event repository_ids", len(event_repo_ids)),
         ("orphan events", orphan_events), ("unused repo rows", unused_repo)],
        "Search Visibility -- repository_id coverage & integrity", "", "count", rot=30)
barplot(per_repo_pairs, "Search Visibility -- events per repository_id (top 30)", "repository_id", "events", rot=90)

# COMMAND ----------

# DBTITLE 1,Findings
print(f"repository rows={repo_total}  distinct ids={len(repo_ids)}  key_unique={repo_key_unique}")
print(f"events distinct repository_id~={S['events_distinct_repo']}  matched rows={matched}/{ev_total}")
