...
RESOLUTION_SECONDS = {
    "quarterhour": 900, "quarter_hour": 900, "15min": 900,
    "hour": 3600, "hourly": 3600, "day": 86400, "daily": 86400, "week": 604800,
}
...
def as_ts(col: str):
    ...
    return F.coalesce(F.to_timestamp(c), (c.cast("double") / 1000).cast("timestamp"),
...
for c in COLS:
    print(f"  {c:<14} missing={r[c + '__m']:>12} rate={r[c + '__m'] / total:.4f} approx_distinct={acd[c]}")
...
combos = df.groupBy("metric", "region", "resolution").agg(
    F.count(F.lit(1)).alias("rows"), F.approx_count_distinct("filter_id").alias("filter_ids")
).collect()
...
missing_combos = [(m, rg, rs) for m in metrics for rg in regions for rs in resolutions
...
print(f"metrics={len(metrics)} regions={len(regions)} resolutions={len(resolutions)}  "
...
dk = df.groupBy(*SERIES_KEY, "timestamp_utc").agg(
    F.count(F.lit(1)).alias("n"), F.countDistinct("value").alias("value_variants"),
)
db = dk.agg(
    F.count(F.lit(1)).alias("series_ts_keys"),
    F.sum((F.col("n") > 1).cast("long")).alias("dup_groups"),
    F.sum(((F.col("n") > 1) & (F.col("value_variants") > 1)).cast("long")).alias("conflicting"),
).first().asDict()
series = df.groupBy(*SERIES_KEY).agg(
    F.count(F.lit(1)).alias("rows"),
    F.min("timestamp_utc").alias("min_ts"), F.max("timestamp_utc").alias("max_ts"),
    F.countDistinct("timestamp_utc").alias("distinct_ts"),
).orderBy(*SERIES_KEY).collect()
series_rows = [("|".join(str(x[k]) for k in SERIES_KEY), x["rows"]) for x in series]
print(f"distinct series = {len(series)}")
print("(series, ts) dup groups:", db["dup_groups"], " conflicting values:", db["conflicting"])

# COMMAND ----------

# DBTITLE 1,Value stats + filter_id cardinality per metric (one groupBy)
v = F.col("value").cast("double")
bm = df.groupBy("metric").agg(
    F.min(v).alias("min"), F.max(v).alias("max"), F.avg(v).alias("mean"), F.stddev(v).alias("sd"),
    F.expr("percentile_approx(cast(value as double), array(0.01,0.25,0.5,0.75,0.99))").alias("p01_25_50_75_99"),
    F.sum((v == 0).cast("long")).alias("zero_rows"), F.sum((v < 0).cast("long")).alias("negative_rows"),
    F.sum((F.col("value").isNull() | (F.trim(F.col("value")) == "")).cast("long")).alias("missing"),
    F.sum((F.col("value").isNotNull() & (F.trim(F.col("value")) != "") & v.isNull()).cast("long")).alias("non_numeric"),
...
for m, x in by_metric.items():
    if x["sd"] and x["sd"] > 0:
        oe.append(F.sum(((F.col("metric") == m) & (F.abs(v - x["mean"]) > 5 * x["sd"])).cast("long")).alias(m))
...
sd = (
    ...
    .where(F.col("ts").isNotNull() & F.col("step").isNotNull()).distinct()
    .withColumn("gap_steps", (F.col("ts").cast("long") - F.lag("ts").over(w).cast("long")) / F.col("step") - 1)
    .groupBy(*SERIES_KEY, "step").agg(
        F.min("ts").alias("min_ts"), F.max("ts").alias("max_ts"), F.count(F.lit(1)).alias("observed"),
        F.max(F.when(F.col("gap_steps") > 0, F.col("gap_steps"))).alias("longest_gap"),
        F.sum(F.when(F.col("gap_steps") > 0, F.col("gap_steps")).otherwise(0)).alias("missing_steps"),
    )
).collect()
continuity = []
for x in sd:
    ...
    continuity.append({
        "series": "|".join(str(x[k]) for k in SERIES_KEY), "resolution": x["resolution"],
        "observed": x["observed"], "expected": exp,
        "coverage_pct": round(x["observed"] / exp * 100, 2) if exp else None,
        "longest_gap": x["longest_gap"], "missing_steps": x["missing_steps"],
    })
    print(continuity[-1])
...
ym = df.groupBy(F.year(as_ts("timestamp_utc")).alias("year"),
                F.date_format(as_ts("timestamp_utc"), "yyyy-MM").alias("month")).count().collect()
by_year = {}
by_month = {}
for x in ym:
...
value_pdf = (
    df.select("metric", v.alias("value")).where(v.isNotNull())
    .sample(0.1, seed=42).limit(200_000).toPandas()
)
ts_pdf = {}
for m in metrics:
    ts_pdf[m] = (
        df.where(F.col("metric") == m).select("timestamp_utc", v.alias("value"))
        .orderBy("timestamp_utc").limit(3000).toPandas()
    )
...
barplot(series_rows, "SMARD -- rows per series", "series", "rows", rot=90, figsize=(12, 5))
barplot(sorted(by_year.items()), "SMARD -- rows per year", "year", "rows", rot=45)
if continuity:
    barplot([(c["series"], c["coverage_pct"]) for c in continuity if c["coverage_pct"] is not None],
            "SMARD -- per-series temporal coverage %", "series", "%", rot=90, figsize=(12, 5))
    barplot([(c["series"], c["longest_gap"]) for c in continuity if c["longest_gap"]],
            "SMARD -- per-series longest gap (missing steps)", "series", "steps", rot=90, figsize=(12, 5))
...
for m in metrics:
    ...
    if len(s):
        histplot(s.tolist(), f"SMARD {m} -- value distribution (n={len(s)} sample)", "value")
    pdf = ts_pdf[m]
    if not pdf.empty:
        plt.figure(figsize=(12, 3))
        plt.plot(range(len(pdf)), pdf["value"], linewidth=0.7)
        plt.title(f"SMARD {m} -- first 3000 points"); plt.xlabel("time index"); plt.ylabel("value")
        plt.tight_layout(); plt.show()
...
grid = np.array([[1 if (m, rg, rs) in present else 0 for rs in resolutions]
                 for m in metrics for rg in regions], dtype=float)
...
plt.title("SMARD -- metric|region x resolution presence"); plt.tight_layout(); plt.show()

# COMMAND ----------

# DBTITLE 1,Findings
print("constant columns:", constant_cols)
print("exact duplicates:", total - distinct_rows, " conflicting (series,ts) keys:", db["conflicting"])
print("distinct series:", len(series), " absent (metric,region,resolution) combos:", len(missing_combos))
print("per-series coverage % (sample):", [c["coverage_pct"] for c in continuity if c["coverage_pct"] is not None][:20])
print("5-sigma outliers per metric:", outliers)
print("value ranges / zero / negative per metric:",
      {m: {k: x[k] for k in ("min", "max", "zero_rows", "negative_rows")} for m, x in by_metric.items()})
