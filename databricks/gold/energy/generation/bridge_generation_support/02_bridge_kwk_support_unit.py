...
# ///
# MAGIC %md
# MAGIC # GOLD -- BRIDGE_KWK_SUPPORT_UNIT
# MAGIC
...
# MAGIC
# MAGIC **Sources:** `mastr_kwk_support_unit_bridge` (Silver, energy_silver_reference); `dim_generation_unit`
...
# MAGIC
# MAGIC **Purpose:** re-key `mastr_kwk_support_unit_bridge` against `dim_generation_unit`'s surrogate key -- `linked_id` = `unit_id`, resolved to `generation_unit_key`.
...
SOURCE = "mastr"
COMPONENT = (
    "gold/energy/generation/bridge_generation_support/02_bridge_kwk_support_unit"
)
RID = gold_run_id()
GOLD_TABLE = "bridge_kwk_support_unit"

# COMMAND ----------

# DBTITLE 1,Read Silver -- mastr_kwk_support_unit_bridge
_bridge_silver = read_silver("mastr_kwk_support_unit_bridge")
...
write_gold_findings(
    SOURCE,
    "bridge_generation_support__bridge_kwk_support_unit",
    GOLD_TABLE,
    _findings_blocks,
)
