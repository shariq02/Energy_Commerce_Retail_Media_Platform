...
# ///
# MAGIC %md
# MAGIC # GOLD -- FACT_UNIT_DELETION_EVENT
# MAGIC
...
# MAGIC
# MAGIC **Sources:** `mastr_unit_deletion_events` (Silver, energy_silver); `dim_generation_unit` (Gold).
# MAGIC
...
# MAGIC
# MAGIC **Purpose:** promote `mastr_unit_deletion_events` to Gold. Grain: one row per `unit_id`. Per `mappings/mastr.yml`, deletion/change events
...
SOURCE = "mastr"
COMPONENT = "gold/energy/grid/fact_unit_lifecycle_event/01_fact_unit_deletion_event"
RID = gold_run_id()
GOLD_TABLE = "fact_unit_deletion_event"

# COMMAND ----------

# DBTITLE 1,Read Silver -- mastr_unit_deletion_events
_silver = read_silver("mastr_unit_deletion_events")
...
fact = fact.withColumn("unit_deletion_event_key", surrogate_key("unit_id"))
fact = add_gold_provenance(fact, SOURCE, RID)

# COMMAND ----------

# DBTITLE 1,Grain assertion -- one row per unit_id
assert_unique_grain(fact, ["unit_id"], component=COMPONENT, source=SOURCE, rid=RID)
...
_findings_blocks = inspect_gold_table(
    ...
    df_before=_silver,
    extra_checks={
        "matched_live_unit_count": fact.filter(
...
write_gold_findings(
    SOURCE,
    "fact_unit_lifecycle_event__fact_unit_deletion_event",
    GOLD_TABLE,
    _findings_blocks,
)
