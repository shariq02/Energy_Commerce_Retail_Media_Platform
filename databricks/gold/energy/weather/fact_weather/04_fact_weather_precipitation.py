...
# ///
# MAGIC %md
# MAGIC # GOLD -- FACT WEATHER -- DWD_PRECIPITATION
# MAGIC
...
# MAGIC
# MAGIC **Sources:** `dwd_precipitation` (Silver, energy_silver); `dim_weather_station`
# MAGIC (Gold, `energy/weather/01_dim_weather_station.py`).
# MAGIC
# MAGIC **Serves use case:** any Energy use case needing `dwd_precipitation` observations
...
SOURCE = "dwd"
COMPONENT = "gold/energy/weather/fact_weather/04_fact_weather_precipitation"
RID = gold_run_id()
GOLD_TABLE = "fact_weather_precipitation"
...
write_gold(precipitation, GOLD_TABLE, source=SOURCE, component=COMPONENT, rid=RID)
...
write_gold_findings(
    SOURCE,
    "fact_weather__fact_weather_precipitation",
    GOLD_TABLE,
    _findings_blocks,
)
