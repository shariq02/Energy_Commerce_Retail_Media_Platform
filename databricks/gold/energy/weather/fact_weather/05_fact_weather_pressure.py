...
# ///
# MAGIC %md
# MAGIC # GOLD -- FACT WEATHER -- DWD_PRESSURE
# MAGIC
...
# MAGIC
# MAGIC **Sources:** `dwd_pressure` (Silver, energy_silver); `dim_weather_station`
# MAGIC (Gold, `energy/weather/01_dim_weather_station.py`).
# MAGIC
# MAGIC **Serves use case:** any Energy use case needing `dwd_pressure` observations
...
SOURCE = "dwd"
COMPONENT = "gold/energy/weather/fact_weather/05_fact_weather_pressure"
RID = gold_run_id()
GOLD_TABLE = "fact_weather_pressure"
...
write_gold(pressure, GOLD_TABLE, source=SOURCE, component=COMPONENT, rid=RID)
...
write_gold_findings(
    SOURCE,
    "fact_weather__fact_weather_pressure",
    GOLD_TABLE,
    _findings_blocks,
)
