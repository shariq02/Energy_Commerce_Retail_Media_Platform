...
# ///
# MAGIC %md
# MAGIC # GOLD -- FACT WEATHER -- DWD_SOIL_TEMPERATURE
# MAGIC
...
# MAGIC
# MAGIC **Sources:** `dwd_soil_temperature` (Silver, energy_silver); `dim_weather_station`
# MAGIC (Gold, `energy/weather/01_dim_weather_station.py`).
# MAGIC
# MAGIC **Serves use case:** any Energy use case needing `dwd_soil_temperature` observations
...
SOURCE = "dwd"
COMPONENT = "gold/energy/weather/fact_weather/09_fact_weather_soil_temperature"
RID = gold_run_id()
GOLD_TABLE = "fact_weather_soil_temperature"
...
write_gold(soil_temperature, GOLD_TABLE, source=SOURCE, component=COMPONENT, rid=RID)
...
write_gold_findings(
    SOURCE,
    "fact_weather__fact_weather_soil_temperature",
    GOLD_TABLE,
    _findings_blocks,
)
