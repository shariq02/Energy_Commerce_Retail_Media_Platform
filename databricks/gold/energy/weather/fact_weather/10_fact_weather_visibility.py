...
# ///
# MAGIC %md
# MAGIC # GOLD -- FACT WEATHER -- DWD_VISIBILITY
# MAGIC
...
# MAGIC
# MAGIC **Sources:** `dwd_visibility` (Silver, energy_silver); `dim_weather_station`
# MAGIC (Gold, `energy/weather/01_dim_weather_station.py`).
# MAGIC
# MAGIC **Serves use case:** any Energy use case needing `dwd_visibility` observations
...
SOURCE = "dwd"
COMPONENT = "gold/energy/weather/fact_weather/10_fact_weather_visibility"
RID = gold_run_id()
GOLD_TABLE = "fact_weather_visibility"
...
write_gold(visibility, GOLD_TABLE, source=SOURCE, component=COMPONENT, rid=RID)
...
write_gold_findings(
    SOURCE,
    "fact_weather__fact_weather_visibility",
    GOLD_TABLE,
    _findings_blocks,
)
