...
# ///
# MAGIC %md
# MAGIC # GOLD -- FACT WEATHER -- DWD_EXTREME_WIND
# MAGIC
...
# MAGIC
# MAGIC **Sources:** `dwd_extreme_wind` (Silver, energy_silver); `dim_weather_station`
# MAGIC (Gold, `energy/weather/01_dim_weather_station.py`).
# MAGIC
# MAGIC **Serves use case:** any Energy use case needing `dwd_extreme_wind` observations
...
SOURCE = "dwd"
COMPONENT = "gold/energy/weather/fact_weather/13_fact_weather_extreme_wind"
RID = gold_run_id()
GOLD_TABLE = "fact_weather_extreme_wind"
...
write_gold(extreme_wind, GOLD_TABLE, source=SOURCE, component=COMPONENT, rid=RID)
...
write_gold_findings(
    SOURCE,
    "fact_weather__fact_weather_extreme_wind",
    GOLD_TABLE,
    _findings_blocks,
)
