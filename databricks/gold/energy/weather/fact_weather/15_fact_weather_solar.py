...
# ///
# MAGIC %md
# MAGIC # GOLD -- FACT WEATHER -- DWD_SOLAR
# MAGIC
...
# MAGIC
# MAGIC **Sources:** `dwd_solar` (Silver, energy_silver); `dim_weather_station`
# MAGIC (Gold, `energy/weather/01_dim_weather_station.py`).
# MAGIC
# MAGIC **Serves use case:** any Energy use case needing `dwd_solar` observations
...
SOURCE = "dwd"
COMPONENT = "gold/energy/weather/fact_weather/15_fact_weather_solar"
RID = gold_run_id()
GOLD_TABLE = "fact_weather_solar"
...
write_gold(solar, GOLD_TABLE, source=SOURCE, component=COMPONENT, rid=RID)
...
write_gold_findings(
    SOURCE,
    "fact_weather__fact_weather_solar",
    GOLD_TABLE,
    _findings_blocks,
)
