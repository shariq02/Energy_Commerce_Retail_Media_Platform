...
for _schema in SILVER_SCHEMAS:
    ...
    for t in tables:
        try:
            schema = spark.table(f"{CATALOG}.{_schema}.{t}").schema
        except Exception as exc:
            print(f"  SKIP {_schema}.{t}: {exc}")
            continue
        for i, f in enumerate(schema.fields, start=1):
            rows.append(
                (_schema, t, i, f.name, f.dataType.simpleString(), bool(f.nullable))
            )
...
def _canonical(schema_rows):
    return "\n".join(
        f"{s}\t{t}\t{o}\t{c}\t{d}\t{int(n)}" for s, t, o, c, d, n in sorted(schema_rows)
    )
...
def _parse_snapshot(path):
    """Return (hash, [(schema, table, ordinal, column, datatype, nullable)])
    from a previously written snapshot Markdown file."""
...
def _diff(prev, curr):
    prev_by_st = {}
    curr_by_st = {}
    for s, t, o, c, d, n in prev:
        prev_by_st.setdefault((s, t), {})[c] = (o, d, n)
    for s, t, o, c, d, n in curr:
        curr_by_st.setdefault((s, t), {})[c] = (o, d, n)
    added_t = sorted(set(curr_by_st) - set(prev_by_st))
    removed_t = sorted(set(prev_by_st) - set(curr_by_st))
    per_table = []
    for st in sorted(set(prev_by_st) & set(curr_by_st)):
        pc, cc = prev_by_st[st], curr_by_st[st]
        notes = []
...
if next_version == 1:
    ...
    _summary.append("")
    _summary.append(
        f"**Tables added:** {', '.join(f'{s}.{t}' for s, t in _added_t) or '(none)'}"
    )
    _summary.append(
        f"**Tables removed:** {', '.join(f'{s}.{t}' for s, t in _removed_t) or '(none)'}"
    )
    for (s, t), notes in _per_table:
        _summary.append("")
        _summary.append(f"**{s}.{t}**")
        for nline in notes:
...
for s, t, o, c, d, n in sorted(rows):
    lines.append(f"| {s} | {t} | {o} | {c} | {d} | {'true' if n else 'false'} |")
lines.append("")
