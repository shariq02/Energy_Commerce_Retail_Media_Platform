...
# ///
# MAGIC %md
# MAGIC # SILVER -- MASTR_LOCATION_CONNECTION_BRIDGE
# MAGIC
...
# MAGIC
# MAGIC **Purpose:** the additive `(parent_id, linked_id)` bridge from `mastr_lokationen`'s `NetzanschlusspunkteMaStRNummern` link array. Never replaces `mastr_lokationen` (see `01_mastr_lokationen.py`). One of 7 sibling notebooks in this folder, split
...
SOURCE = "mastr"
COMPONENT = "silver/energy/grid/mastr_grid_topology/06_bridge_location_connection"
RID = run_id()
