...
# MAGIC city hierarchy (DWD Bundesland and AGS code); DWD relocation history in
# MAGIC `weather_location_validity`.
...
COMPONENT = "silver/energy/weather/01_weather_location"
RID = run_id()
...
# DBTITLE 1,Configuration -- DWD station city to Bundesland (curated, 28 stations)
CITY_REGION = {
    "berlin": "Berlin",
    "hamburg": "Hamburg",
    "munich": "Bayern",
...
dwd_validity = (
    dwd_geo_bronze.withColumn("source_location_id", strip_float_suffix("Stations_id"))
...
# Relocations can share a start date (2-17 rows): ordinal, never a drop.
dwd_validity = within_group_ordinal(
    dwd_validity,
...
# DBTITLE 1,Write Quarantine -- station coordinates outside the Germany bounding box
write_quarantine(
    _q_rows(
        dwd_validity.filter(F.array_contains("quality_flags", "coord_outside_de_bbox")),
...
dwd_locations = (
    ...
    .filter(F.col("_rn") == 1)
    .drop("_rn", "valid_from", "valid_to", "record_ordinal", "quality_flags")
...
weather_location_validity = conform(
    add_semantic_provenance(
        ...
        "source_record_id",
        sha_key("source_location_id", "valid_from", "record_ordinal"),
    ),
...
validity_blocks = inspect_table(
    ...
    component=COMPONENT,
    rid=RID,
    key_cols=["location_key", "valid_from", "record_ordinal"],
