...
# MAGIC %run ../iot/honda_energy/_honda_energy_common
...
SOURCE = "honda_iot"
COMPONENT = "silver/energy/weather/03_weather_observation_honda"
RID = run_id()
BRONZE_TABLE = "honda_iot_weather"
SITE_ID = "honda_site"
STUCK_LOOKBACK_STEPS_BY_FREQ = {"1h": 9, "15min": 39, "1min": 599}
# Bronze column -> short name used for the stuck-reading flag.
FLAGGED = {"WeatherStation_Weather_Ta": "ta", "WeatherStation_Weather_Igm": "igm"}
...
for _src, _name in FLAGGED.items():
    honda_base = honda_base.withColumn(_name, F.col(_src).cast("double"))
honda_base = add_stuck_reading_flags(
    honda_base, list(FLAGGED.values()), STUCK_LOOKBACK_STEPS_BY_FREQ, _w
)
...
honda_temperature = any_present(
    honda_base.withColumn("air_temperature_degc", F.col("ta")).withColumn(
        "quality_flags", flag_array({"stuck_reading": F.col("_ta_stuck_reading_flag")})
    ),
    ["air_temperature_degc"],
)
honda_solar = any_present(
    honda_base.withColumn("global_radiation_w_per_m2", F.col("igm")).withColumn(
        "quality_flags",
        flag_array({"stuck_reading": F.col("_igm_stuck_reading_flag")}),
    ),
