...
# concern -- all chunks written by one writer instance remain one
# logical dataset and one Volume upload unit; chunk count must never be
# read as dataset count.
#
# Each write() call is handed a batch no larger than one processing
# batch (<=300,000 rows, per the memory guard). The full dataset is
# never held in RAM: only the one incoming batch plus its own encoded
# copy.
#
# ChunkedJSONLWriter below is for sources whose staged records carry
# nested/variable structure that a fixed CSV column order can't hold.
...
class ChunkedJSONLWriter:
    """Same chunk-rotation contract as ChunkedCSVWriter, writing
    newline-delimited JSON instead -- no header, no locked column order,
    so records can carry nested/variable structure."""
    ...
    def _open_new_chunk(self) -> None:
        ...
        self._chunk_index += 1
        self._current_path = self._chunk_path(self._chunk_index)
        self._current_file = open(  # noqa: SIM115 -- handle stays open across writes, closed on rotation/close()
    ...
    def write(self, records: list[dict]) -> None:
        """Write one processing batch (<=max_rows records), rotating to
        a new chunk file at the row or byte cap. Holds only the batch
        handed in plus one encoded line at a time."""
        for record in records:
            if self._current_file is None or self._current_rows >= self.max_rows:
                self._open_new_chunk()
